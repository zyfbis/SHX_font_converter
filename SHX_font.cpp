...
void Stat_t::add_line(double x1, double x2, double y1, double y2)
{
    ...
    if ((font_is_empty) || (by > font_max_y))
        font_max_y = by;

    double sy = (y1 < y2) ? y1 : y2;
    if ((font_is_empty) || (sy < font_min_y))
        font_min_y = sy;
...
bool SHX_Font_t::load_font(char *file_name)
{
    ...
    if (!(shapes || big_font || uni_font))
        Debug.print("Can't read header\n");
    ...
    for (int sn = 0; sn < total_chars; sn++)
    {
        ...
        { // Non-zero shapes
            ...
            if ((real_sn % 32 == 31) || (real_sn == (total_chars - 1)))
                Debug.print("\n");
            //--- Print finish
...
int SHX_Font_t::convert_to_unicode(void)
{
    ...
    int encoding = (CP1251) ? 1251 : (CP866) ? 866
                                             : 0; // cp1251 is "better"
    ...
    for (int i = 0; i < total_chars; i++)
    {
        ...
        else if (cur_shape < 256)
        { // Cyrilic CP1251 or CP866
            if (encoding == 1251)
...
void SHX_Font_t::sort_symbols(void)
{
    for (int i = total_chars - 1; i > 0; i--)
    {
        for (int j = 0; j < i; j++)
        {
            if (symbol_desc[j].unicode_number > symbol_desc[j + 1].unicode_number)
            { // change
...
void SHX_Font_t::print_shape(int cn)
{
    ...
    for (int i = 0; i < total_chars; i++)
    {
        if (symbol_desc[i].shape_number == cn)
            sn = i;
    }
    if (sn < 0)
    {
    ...
    while (1)
    {
        ...
        { // goto/line (x,y)
            ...
            if (pen_down)
            {
                ...
                {
                    if (draw)
                        fprintf(f, "%f,%f", save_x, save_y);
                    line_started = true;
                }
                if (draw)
                    fprintf(f, ";%f,%f", current_x, current_y);
        ...
        { // circle (r,oct)
            ...
            if (pen_down)
            {
                ...
                {
                    if (draw)
                        fprintf(f, ";%f,%f,A1.0", current_x, current_y);
                    if (draw)
                        fprintf(f, ";%f,%f,A1.0", save_x, save_y);
        ...
        case 0xB:
        { // fractional arc (start,finish,rh,rl,oct)
            ...
            while (arc_angle <= -2048)
                arc_angle += 2048; // 8*256
            while (arc_angle >= 2048)
                arc_angle -= 2048; // 8*256
            if ((clock_wize) && (arc_angle > 0))
                arc_angle -= 2048;
            if ((!clock_wize) && (arc_angle < 0))
                arc_angle += 2048;
...
bool SHX_Font_t::save_lff(char *file_name)
{
    ...
    for (int gstep = 0; gstep < 2; gstep++)
    {
        ...
        for (int sn = 0; sn < total_chars; sn++)
        {
            ...
            if (shnum == 0x0D)
                continue; // Skip 0x0D
            if (symbol_desc[sn].used_bytes == 0)
                continue; // Skip empty
            ...
            if (sn != ' ')
                stat.finish_sym(current_x, current_y);
            if (sn == ' ')
                stat.process_space(current_x, current_y);
            //=== Finish draw symbol
        }
        stat.finish_font();
    }
